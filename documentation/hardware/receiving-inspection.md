...
| A.13 | Ceramic capacitors, 0.1 µF `104` | TBD | 20 | **~20, counted by hand 2026-09-06** | No visible damage | Second batch. Orange discs, **print worn illegible**. Seven are consumed by the build, so there is ample margin. See [D.7](#d7--the-capacitors) |
...
| **0.1 µF ceramic disc** | **Print worn illegible.** The value comes from the purchase, not from the part | One at every module's supply pins, and one at the `GP26` divider tap — seven of them |
...
**Seven `104` ceramics are consumed by the build** — microSD, RA-02, MPU-6500, BMP280, NEO-6M, the sound board, and the `GP26` divider tap, which needs one for a reason that is not decoupling at all ([Signal routing](assembly-procedure.md#signal-routing-and-why-spacing-is-the-wrong-lever)) — against about twenty in hand, so there is ample margin. They are the one part here with no substitute, since a bulk capacitor of any type is too slow for the edges they cover.
