...
| 32 | **`team_id` is *not* the placeholder, and that is the problem.** `main.cpp:40` sets `CAN-Team-25`, which **passes** `is_valid_team_id()` — the guard exists to catch `CAN-Team-XX` and has already been satisfied by a value nobody has confirmed against a registration | **Blocks flight, and silently.** The safety net that was supposed to catch an unset identifier will never fire on this one. Confirm the registered number, or nothing will complain again |
