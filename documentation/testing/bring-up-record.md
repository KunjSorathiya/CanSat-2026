...
| 8.17 | **`MAX_RATE` on the air** | ~3.13 Hz at the station; every third packet carries `GP-` and `SN-` and the two between carry neither; a second press does nothing; a power cycle restores 1.43 Hz | Send it from the console in `READY` with `ARM-0`, measure the station's rate over two minutes and read six consecutive raw packets. The vehicle's startup summary must read `rate COMMANDED MAX - uplink closed` and `commands accepted 1` | | |
| 8.18 | **Loss at the commanded rate** | No gaps in packet numbering over two minutes at 3.13 Hz | The rulebook scores rate and packet loss on the same five points, so a rate that costs packets is not a gain. Count gaps in the station's packet numbers | | |
