...
| 6.3 | Single block write time | — | Time 100 `write_block` calls | **2.677 ms mean, 4.814 ms worst**, 100/100 written, on a first pass. **Three sessions: 4.814 ms, 29.756 ms and 4.915 ms worst case**, 100/100 written each time, means of 2.677, 6.160 and 2.678 ms. **Fourth session, soldered board, 2026-09-07: 6.128 ms mean, 28.031 ms worst**, 100/100 written, read-back matched. So the worst case is **4.814, 29.756, 4.915 and 28.031 ms** across four sessions — **~30 ms has now been seen twice, not once.** It is not a one-off, and [F-11](#findings) is updated accordingly. The worst case is the number that matters for flight | ✅ 2026-09-07 / KS |
| 6.3b | **Sustained write load** | — (this is the Gate 2 input) | Meter in series for a current; **or DC volts across the rail for the go/no-go** | **2979–3672 writes in 10.0 s across three sessions — 297 to 367 writes/s, 148.6 to 183.6 KiB/s, at 100.0 % duty.** Current not taken; the series connection would not hold. **Instead the 3V3 rail was watched under that load and held 3.28–3.30 V.**<br><br>**Fourth session, soldered board, 2026-09-07: 3598 writes in 10.0 s — 360 writes/s, 179.9 KiB/s, 100.0 % duty**, at the top of the range seen. **No current was taken on this run either**, so the figure this row exists for is still owed after four attempts | ⚠️ 2026-09-07 / KS — throughput settled, current never once measured |
...
| 12 | 6 · 7 | The same card that initialised in **32 ms** on 2026-09-05 to do so again | **It did both, in two runs eleven minutes apart with nothing changed between them.** The first timed out at 2007 ms in ACMD41 with `R1 = 0xFF`; the second completed in **31 ms, 1 CMD0 attempt**, then wrote 100/100, sustained 3598 writes in 10 s at 100 % duty, and passed every row of Gate 7 | **Open, and the pass is not the result — the pair is.** `0xFF` is not a slow card: an initialising card answers `0x01` until it answers `0x00`, so the card had stopped responding altogether. **The supply is measured innocent:** the module's own `3V3` read **3.28 V** during the failing run, and the rail held **3.28–3.29 V** under 45 back-to-back transmits. That leaves a *mechanical* intermittent — the friction-fit card holder, or a joint on the module that moves when the board is handled, and the board was being handled to place meter probes. **This repository has been here twice.** [F-5](#findings) was the radio failing 5/5 then passing 45/45 minutes later; [F-6](#findings) was every write failing on one power cycle and 100/100 on the next. Both looked fixed and both were a marginal supply connection that took five bench runs to name. **Do not close this on one good run.** Next: ten power-cycled runs, counting failures, before anything else is added to the board |
