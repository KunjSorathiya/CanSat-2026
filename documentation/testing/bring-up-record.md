...
| 8.16 | **`MAX_RATE_GPS` on the air** | 364 ms, 2.75 Hz, packets carrying `GP-` and no `MODE`/`FAULTS`/`CAL`/`ARM`/`YR` | Send it from the console in `READY` with `ARM-0`; measure the station's rate over 2 minutes and read one raw packet | | |
| 8.17 | **`MAX_RATE_LEAN` on the air** | 281 ms, 3.56 Hz, mandatory fields only, no position | Same, on a separate power cycle. Also confirm the other button then does nothing and that a power cycle restores 1.43 Hz | | |
| 8.18 | **Loss at the commanded rate** | No gaps in packet numbering over 2 minutes at either rate | The rulebook scores rate and loss on the same five points, so a rate that costs packets is not a gain. Count gaps in `logs/raw_packets.tsv` | | |
