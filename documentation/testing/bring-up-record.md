...
| 15 | 3 · 6 | The microphone's readings to reach the SD log, as rows 3.11 and 3.12 instruct | **They reach nothing.** The sound data is read, validated, fault-managed and counted into `health_.sound_ok` — and is then discarded | **Open, and it makes rows 3.11 and 3.12 untakeable as written.** The chain breaks in two places. `TelemetryBuilder::build()` copies the sound fields into `Built` (lines 81–85) and `sd_line()` renders them (lines 139–143) — but **`sd_line()` and `sd_header()` are called only from `flight_tests.cpp`**, never from the flight path. And `PicoSdLogger::append()` takes `(const TelemetryRecord&, const std::string& packet)` and **writes only `packet`**, discarding its first argument; it never sees `Built` at all. The packet itself carries no sound either — its optional fields are GPS lat/lon/alt plus `MODE`, `FAULTS`, `CAL`, `ARM` and `YR`, and `telemetry.cpp` has no mention of sound. **So the additional sensor this vehicle chose over a magnetometer produces data that reaches neither the radio nor the card.** The interface is the constraint: `append()` would need the rendered `sd_line()`, or `Built`, rather than a packet string |
