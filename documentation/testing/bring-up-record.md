...
| 5.2 | Airtime, 206-byte packet, SF7/125 kHz | **328 ms** | [link-budget.md](../design/link-budget.md) | Scope DIO0, TX start to TxDone | **333.7 ms**, mean of 5, all 5 sent. +5.7 ms (1.7 %) over the 327.9 ms model. **The next power cycle failed all 5 with `IRQ_FLAGS = 0x00`** — see [F-5](#findings); the timing is right when it works and the row stays open until it works every time | ⚠️ 2026-09-05 / KS — timing confirmed, reliability not |
| 5.3 | Airtime, full 255-byte packet | **400 ms** | Same | Same, with a padded packet | **406.9 ms**, +7.3 ms (1.8 %) over the 399.6 ms model. All 5 sent on one power cycle; **2 of 5 on the next, to the same 406.9 ms** — the successful transmissions time identically whether or not their neighbours fail. See [F-5](#findings) | ⚠️ 2026-09-05 / KS — timing confirmed, reliability not |
...
| 7.3 | Radio works *during* card writes | No lost packets | Run both at full rate for 5 minutes | **30 transmit-then-write rounds: 0 transmit failures, 0 write failures, 0 radio misreads.** Each round is a 206-byte transmit, then a block write, then a radio version read — the interleaving the flight loop actually does. Ten seconds, not five minutes | ⚠️ 2026-09-05 / KS — passes; the duration this row asks for is still owed |
...
| 5 | 5 | A radio that either works or does not | **The same 206-byte transmit failed 5/5 in row 5.2 and then succeeded 45/45 in row 5.4a, minutes later on one power cycle** — same code, same length, same antenna. The previous power cycle had passed 5.2 and 5.3 outright. At the failures `IRQ_FLAGS = 0x00`: `TxDone` was never set, so the packets did not merely go unreported, they did not complete | Not closed. `IRQ_FLAGS` alone cannot separate a chip that is trying and failing from one that has been reset out of LoRa mode, and those want opposite investigations. The driver now also captures **`RegOpMode` and `RegVersion` at the moment of the failure**, and the diagnostic decodes them: a version that is not `0x12` means SPI was broken at that instant, `OP_MODE` bit 7 clear means the module reset and lost its configuration, `0x83` means it took the job and never finished it. Re-run and read that line before touching anything |
...
| 6 | 6 · 7 | Nothing — this row exists because the previous run's failure vanished | **The previous power cycle failed every write (row 6.6 `logger init FAILED`, row 7.3 30 write failures) while all 200 reads passed. The next power cycle, with an unchanged write path, wrote 100/100 in 6.3, 2667 in 6.3b and 30/30 in 7.3** | The write path was instrumented but **not changed** between the two runs — the commit added `WriteStage`, the R1/R2/token capture and a located LBA in place of a remembered one, none of which alter what is sent to the card. So the fault was outside the code, and it went away without being understood. Recorded rather than closed: an intermittent that disappears is still an intermittent, and this one and [F-5](#findings) are on the same SPI0 bus |
