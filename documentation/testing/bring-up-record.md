...
| # | Quantity | Predicted | How to measure | Measured | Verdict |
|---|---|---|---|---|---|
| 6.1 | Card initialises | CMD0/CMD8/ACMD41 succeed | `sd_ok` in the health snapshot, or `cansat_bringup_firmware` | **Complete on the breadboard.** 32 ms in ACMD41, 1 CMD0 attempt, on an HP mx310 64 GB. Reached only after [the CMD0 fix](#gate-6--storage) - the first attempt on this card returned `0x1F`.<br><br>**Regressed on the soldered board, 2026-09-07: ACMD41 timed out at 2008 ms with `R1 = 0xFF`.** Open, and under investigation as [F-12](#findings) | ❌ 2026-09-07 / KS — passed on the breadboard, fails on the soldered board |
...
| 12 | 6 | The same card that initialised in **32 ms** on 2026-09-05 to do so again | **ACMD41 timed out at 2008 ms with `R1 = 0xFF`**, on the soldered board, after CMD0 and CMD8 both answered. An earlier run the same evening stopped further back, at CMD0 | Open. **`0xFF` is the whole diagnosis and it is not slowness:** a card that is initialising answers `0x01` until it answers `0x00`, so `0xFF` means MISO was undriven and the card had stopped responding altogether. That is [F-8](#findings)'s signature one command earlier — ACMD41 is the first moment the card powers its own controller and draws real current. **Two things are ruled out.** The HCS/SDXC trap is not it: `sd_card.cpp` sets `0x40000000` when CMD8 validates, and a 64 GB card denied HCS answers `0x01` forever rather than `0xFF`. The 3.3 V rail is not it either: the radio pulled ~120 mA sustained through 45 consecutive transmits at 100 % in the same run. **So the fault is downstream of the 3.3 V distribution node** — the module's own supply pair, its solder joints, or the card contacts. Next measurement is the module's own `3V3` against `GND` during the 2 s ACMD41 window |
