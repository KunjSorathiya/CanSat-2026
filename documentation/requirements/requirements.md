...
| GEN-005 | The CanSat must remain within the applicable mass limit. | Rulebook 2026 - General Rules | Mandatory | **500 g (+/-10%).** Exceeding by >10% is a disqualification. **Whether the lower edge binds is open** — the +/-10% reads as a 450-550 g band, but the disqualification condition names only *exceeding*, and the projected all-up mass of 414-479 g straddles the 450 g edge (question 12) | Weigh the completed CanSat on a documented scale. | **Requirement locked; avionics built, structure not** — the vehicle board is complete and working as of 2026-09-07; nothing in `mechanical/` exists yet, so neither dimension nor mass can be measured | |
...
| PWR-001 | The CanSat must have a manual ON/OFF switch. | Rulebook - Power / Functional Requirements | Mandatory | **Part held since 2026-09-06** — an I/O switch, confirmed in the [purchase list](../hardware/purchase-list.md). **Not fitted**, but it now has a home: the structure carries a **rectangular cutout for it**, confirmed 2026-09-09, with the two indicator LEDs beside it. Its place in the power path is drawn: battery → switch → Schottky → Pico `VSYS` ([netlist](../../electrical/schematics/vehicle-netlist.tsv), nets `VBAT`/`VSW`) | Inspect hardware and perform repeated power-cycle test. | Not Started | Nothing blocks fitting it. It is also the only control behind [TEL-026](../operations/runbook.md#radio-silence--when-your-vehicle-must-be-off), so its absence costs more than its own five points |
