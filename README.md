...
[![C++ tests](https://img.shields.io/badge/C%2B%2B%20tests-4430%20assertions-1b5e20)](documentation/testing/test-plan.md)
...
| 🟠 **Hardware** | **The vehicle board is soldered and Gates 5, 6 and 7 pass on it** — **30 of 87 recorded measurements taken.** The radio transmitted 5/5, 5/5 and 45/45 back to back with airtimes within 1.8 % of the model; the card initialised in 31 ms, wrote 100/100 and sustained 3598 writes in 10 s; the shared SPI0 bus passed every row including 200 interleaved rounds with zero misreads. **Power is largely answered:** the Pico's own 3.3 V rail held **3.28–3.29 V through 45 back-to-back transmits** and 3.28–3.30 V at 100 % microSD write duty, both far harsher than the mission, so no separate rail is needed. [F-5 and F-6 are closed](documentation/testing/bring-up-record.md#findings) — both were supply jumpers, and soldering removed them. **[F-12] is open:** the card failed three of its first four runs on the soldered board and has passed eleven since, with the supply measured innocent, so what remains is mechanical and eleven clean runs only bound it. Still missing: the four jumpered modules, the switch, the divider, and the Schottky that stops USB back-powering the pack |
...
| Documented claims | Numbers in the documentation checked against the source that defines them, test counts included | ✅ **218 / 218** |
